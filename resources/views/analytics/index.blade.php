...
<div class="container-fluid analytics-page">
    ...
    <div class="card"><div class="card-header"><i class="fas fa-money-check-alt text-success me-2"></i>Payroll Totals for Selected Period</div><div class="card-body"><div class="row text-center"><div class="col-md-4"><div class="text-muted">Gross Pay</div><div id="grossPay" class="fw-bold fs-4">₱{{ number_format($analytics['payroll']['gross'], 2) }}</div></div><div class="col-md-4"><div class="text-muted">Deductions</div><div id="deductions" class="fw-bold fs-4 text-danger">₱{{ number_format($analytics['payroll']['deductions'], 2) }}</div></div><div class="col-md-4"><div class="text-muted">Net Pay</div><div id="netPay" class="fw-bold fs-4 text-success">₱{{ number_format($analytics['payroll']['net'], 2) }}</div></div></div></div></div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    setInterval(async function () { 
        ...
        const response = await fetch('{{ route('analytics.data') }}?' + params.toString(), { headers: { Accept: 'application/json' } }); 
