                ...
                <div class="card-body">
                    ...
                    <div class="alert alert-info mb-4">
                        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2 mb-3">
                            <h6 class="mb-0"><i class="fas fa-chart-line"></i> Your Leave Balances ({{ date('Y') }})</h6>
                            <span class="badge bg-primary fs-6">Status: {{ $profile->status ?? 'New Hire' }}</span>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-lg-6">
                            <form id="leaveRequestForm" method="POST" action="{{ route('leave.store') }}">
                                @csrf
                                <div class="row">
                                ...
                                <div class="row">
                                    <div class="col-md-6 mb-3">
                                        ...
                                        <input id="start_date" type="date" name="start_date" class="form-control" min="{{ now()->toDateString() }}" value="{{ old('start_date') }}" required>
                                    </div>
                                    <div class="col-md-6 mb-3">
                                        <label class="form-label">End Date</label>
                                        <input id="end_date" type="date" name="end_date" class="form-control" min="{{ now()->toDateString() }}" value="{{ old('end_date') }}" required>
                                    </div>
                                </div>
                                <div class="mb-3">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-6">
                                        <a href="{{ route('leave.index') }}" class="btn btn-secondary w-100">Cancel</a>
                                    </div>
                                </div>
                            </form>
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const blockedLeaves = [
            ...
            { start: @json($leave->start_date->format('Y-m-d')), end: @json($leave->end_date->format('Y-m-d')) },
            @endforeach
        ];
            ...
            events: [
                @if(isset($calendarEvents) && $calendarEvents->count())
                    @foreach($calendarEvents as $event)
                    {
                        ...
                        backgroundColor: '{{ $event->event_type === 'activity' ? '#0d6efd' : '#fd7e14' }}',
                        borderColor: '{{ $event->event_type === 'activity' ? '#0d6efd' : '#fd7e14' }}',
                        textColor: '#ffffff'
                    },
                    @endforeach
                @endif
                @if(isset($approvedLeaves))
                    @foreach($approvedLeaves as $leave)
                    {
                        id: @json('approved-leave-' . $leave->id),
                        title: @json('Approved ' . ucfirst($leave->leave_type) . ' Leave'),
